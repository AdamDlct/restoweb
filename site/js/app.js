...
const orderId = Math.floor(Math.random() * 9000) + 1000; // Numéro de commande, généré une seule fois au chargement
...
function goTo(page) {
  document.querySelectorAll(".page").forEach((elt) => elt.classList.remove("active"));
  document.getElementById("page-" + page).classList.add("active");
  window.scrollTo(0, 0);
  // À chaque arrivée sur une page dynamique, on (re)calcule son contenu :
...
function addToCart(item) {
  ...
  if (existing) existing.qty += item.qty; // produit déjà présent → on additionne les quantités
  else cart.push(item);                   // sinon on l'ajoute comme nouvelle ligne
  updateCartBadges();
}

function updateQty(id, qty) {
  if (qty <= 0) cart = cart.filter((c) => c.id !== id); // quantité à 0 → on retire l'article
